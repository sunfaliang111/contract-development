...
const relatedProjectCount = computed(() => relatedProjects.value.length)
const assignedProjectCount = computed(() =>
  relatedProjects.value.filter((project) => (project.assignedPersonnelCount || 0) > 0).length
)
const unassignedProjectCount = computed(() => relatedProjectCount.value - assignedProjectCount.value)
const newProjectPath = computed(() => ({
  path: '/projects/new',
  query: {
...
const deleteProject = async (project: Project) => {
  if (!window.confirm(`案件「${project.projectName}」を削除します。よろしいですか？`)) {
    return
  }
  ...
  try {
    ...
    successMessage.value = '案件を削除しました。'
  } catch {
      ...
      <template v-if="customer">
        ...
        <div class="relationship-summary mb-5">
          <div class="relationship-metric">
            ...
            <div class="relationship-metric__value">{{ relatedProjectCount }}</div>
          </div>
          <div class="relationship-metric relationship-metric--success">
            <div class="relationship-metric__label">要員アサイン済</div>
            <div class="relationship-metric__value">{{ assignedProjectCount }}</div>
          </div>
          <div class="relationship-metric relationship-metric--warning">
            <div class="relationship-metric__label">未アサイン</div>
            <div class="relationship-metric__value">{{ unassignedProjectCount }}</div>
          </div>
        </div>
        <div class="d-flex align-center justify-space-between mb-3">
          <h2 class="text-subtitle-1 font-weight-bold">関連案件</h2>
          <v-btn prepend-icon="mdi-plus" variant="outlined" :to="newProjectPath">案件を登録</v-btn>
        </div>
            ...
            <tr v-for="project in relatedProjects" :key="project.id">
              ...
              <td>
                <div class="d-flex ga-1">
                  <v-btn density="comfortable" icon="mdi-pencil" :to="`/projects/${project.id}/edit`" variant="text" />
                  <v-btn color="error" density="comfortable" icon="mdi-delete-outline" variant="text" @click="deleteProject(project)" />
                </div>
              </td>
            </tr>
            <tr v-if="relatedProjects.length === 0">
              <td class="text-center text-medium-emphasis py-6" colspan="6">この取引先に紐づく案件はありません。</td>
            </tr>
          </tbody>
        </v-table>
      </template>
